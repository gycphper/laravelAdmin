...
<table class="layui-table  text-center" lay-size="sm">
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('edit-permission',['id'=>$vo['id']]) }}" class="layui-btn layui-btn-xs layui-btn-normal  ajax-form" title="{{ $vo['title'] }}">修改</a>
                <a href="{{ route('delete-permission',['id'=>$vo['id']]) }}" title="{{ $vo['title'] }}" confirm="1" class="layui-btn layui-btn-xs layui-btn-danger  ajax-get">删除</a>
            </td>
        </tr>
        @endforeach
    </tbody>
